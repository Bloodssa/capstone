<x-admin-layout title="Register New Warranty" subtitle="Assign purchase product to the customer and send email invitation">
    @if (session('status'))
        <x-ui.toast type="success" message="{{ session('status') }}" />
    @endif
    <form method="POST" action="{{ route('register-warranty-details') }}" class="mt-6 space-y-6">
        @csrf
        {{-- Customer Info --}}
