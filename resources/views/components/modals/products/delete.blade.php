...
<div x-show="{{ $data }}" x-cloak
    ...
    <div @click.outside="{{ $data }} = false"
        class="no-scrollbar relative flex w-full max-w-122.5 max-h-[90vh] flex-col overflow-y-auto rounded-xl bg-white p-6 lg:p-11 shadow-2xl">
        ...
        <div class="flex flex-col items-center px-4">
            ...
            <p class="mb-8 text-sm leading-relaxed text-gray-500 max-w-sm mx-auto text-center whitespace-normal wrap-break-words">
                Are you sure you want to delete product: <span class="text-neutral-900 text-md font-semibold">{{ $product->name }}</span>
            </p>
        </div>
        <form method="POST" action="{{ route('delete-product') }}"
            ...
            <button type="submit"
                class="flex w-full sm:w-auto justify-center rounded-md bg-red-400 px-8 py-3 text-sm font-semibold text-white hover:bg-red-500 transition-all active:scale-95">
                Yes, Delete
            </button>
        </form>
    </div>
